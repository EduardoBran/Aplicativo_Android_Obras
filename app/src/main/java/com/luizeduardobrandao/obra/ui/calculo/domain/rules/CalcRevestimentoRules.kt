...
object CalcRevestimentoRules {
    ...
    object PisoVinilico {
        // ============= DIMENSÕES DA PEÇA (RÉGUA/PLACA) =============
        // Comprimento (cm)
        const val COMP_MIN_CM = 10.0
        const val COMP_MAX_CM = 200.0
        const val COMP_DEFAULT_CM = 135.0  // Valor padrão pré-preenchido
        val COMP_RANGE_CM = COMP_MIN_CM..COMP_MAX_CM

        // Largura (cm)
        const val LARG_MIN_CM = 10.0
        const val LARG_MAX_CM = 200.0
        const val LARG_DEFAULT_CM = 24.0  // Valor padrão pré-preenchido
        val LARG_RANGE_CM = LARG_MIN_CM..LARG_MAX_CM

        // ============= RODAPÉ (INFORMADO EM METROS) =============
        // Perímetro de rodapé manual (m)
        const val RODAPE_MIN_M = 0.50
        const val RODAPE_MAX_M = 500.0
        const val RODAPE_DEFAULT_M = 2.0  // Valor padrão quando switch ativo
        val RODAPE_RANGE_M = RODAPE_MIN_M..RODAPE_MAX_M

        // ============= QUANTIDADE DE DEMÃOS (MASSA PVA) =============
        const val QTD_DEMAOS_MIN = 1
        const val QTD_DEMAOS_MAX = 4
        const val QTD_DEMAOS_DEFAULT = 2  // Valor padrão quando switch ativo
        val QTD_DEMAOS_RANGE = QTD_DEMAOS_MIN..QTD_DEMAOS_MAX

        // ============= RENDIMENTOS DE MATERIAIS =============
        // Cola para piso vinílico (NÃO auto-adesivo)
        const val RENDIMENTO_COLA_M2_POR_KG = 3.0
        const val EMBALAGEM_COLA_KG = 5.0

        // Massa PVA para regularização (desnível)
        const val RENDIMENTO_PVA_GROSSO_M2_POR_KG = 1.04  // Piso Grosso
        const val RENDIMENTO_PVA_LISO_M2_POR_KG = 2.04    // Piso Liso
        const val EMBALAGEM_PVA_KG = 25.0

        // ============= ALTURA PADRÃO DO RODAPÉ PARA CÁLCULO =============
        // Usada internamente no PisoVinilicoCalculator para calcular área de rodapé
        const val ALTURA_RODAPE_PADRAO_M = 0.10  // 10 cm
    }
