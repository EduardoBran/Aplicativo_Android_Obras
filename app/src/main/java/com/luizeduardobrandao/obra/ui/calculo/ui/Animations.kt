...
object Animations {
    ...
    /** Rolagem vertical realmente suave até uma coordenada Y. */
    fun smoothScrollToY(scrollableView: View, targetY: Int, baseDuration: Long = 350L) {
        scrollableView.post {
            startSmoothScroll(scrollableView, targetY, baseDuration)
        }
    }

    /** Faz scroll suave até o card/section dentro de uma ScrollView.
     *  - scrollView: normalmente binding.scrollContent
     *  - targetView: ex.: binding.groupPlacaTipo ou binding.groupMgAplicacao
     *  - extraTopMarginDp: margem extra para o card não colar no topo */
    fun smoothScrollToView(
        scrollView: View, targetView: View, baseDuration: Long = 450L, extraTopMarginDp: Int = 16
    ) {
        scrollView.post {
            val parentGroup = scrollView as? ViewGroup ?: return@post

            val rect = Rect()
            // pega o retângulo do targetView relativo a si mesmo
            targetView.getDrawingRect(rect)
            // converte esse retângulo para as coordenadas do scrollView
            parentGroup.offsetDescendantRectToMyCoords(targetView, rect)

            val density = scrollView.resources.displayMetrics.density
            val extraMarginPx = (extraTopMarginDp * density).toInt()

            val targetY = (rect.top - extraMarginPx).coerceAtLeast(0)

            startSmoothScroll(scrollView, targetY, baseDuration)
        }
    }

    /** Implementação compartilhada de scroll suave usada por smoothScrollToY e smoothScrollToView. */
    private fun startSmoothScroll(scrollableView: View, targetY: Int, baseDuration: Long) {
    ...
    /** Garante que a ScrollView volte para o topo sem flicker, cancelando qualquer animação em
     *  curso e usando OnPreDraw para reforçar o scrollTo(0, 0) após o layout.
    ...
    fun ensureScrollAtTopWithoutFlicker(
        scrollView: View, rootView: View, expectedStep: Int,
        shouldHijackFocus: Boolean, currentStepProvider: () -> Int
    ) {
        ...
        scrollView.viewTreeObserver.addOnPreDrawListener(object :
